...
After a successful deployment, all RAG components run inside the `llama-stack-rag` namespace on OpenShift.
...
A healthy deployment will show output similar to the following.
...
[NOTE]
====
The predictor pods show `2/2` ready when running in raw deployment mode.
If `RAW_DEPLOYMENT=false` was set during installation, expect `3/3`.
...
|`llama-guard-3-8b`
|vLLM inference endpoint for the safety/shield model (if deployed)
