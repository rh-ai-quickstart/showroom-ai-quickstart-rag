...
== Try it: query in direct mode

. In the sidebar, set *Mode* to *Direct*.
...
In *xref:04-chat-mode.adoc[Direct mode]*, the query follows a single linear path.
...
|Scanned PDFs require optical character recognition (OCR) pre-processing. Convert them to text-layer PDFs before uploading.
